...
def specPlot(data, Fs=1, maxCode=None, harmonic=7, winType=np.hanning,
    ...
    if isPlot:
        ...
        if label:
            if logSca == 0:
                plt.plot(freq[max(bin_max-sideBin, 0):min(bin_max+sideBin, Nd2)],
                         10 * np.log10(spec[max(bin_max-sideBin, 0):min(bin_max+sideBin, Nd2)]),
                         'r-', linewidth=0.5)
            else:
                plt.semilogx(freq[max(bin_max-sideBin, 0):min(bin_max+sideBin, Nd2)],
                             10 * np.log10(spec[max(bin_max-sideBin, 0):min(bin_max+sideBin, Nd2)]),
                             'r-', linewidth=0.5)
        ...
        if harmonic > 0:
            for i in range(2, harmonic + 1):
                ...
                plt.text(b/N * Fs, 10 * np.log10(spec[b + 1] + 10**(-20)) + 5, str(i),
    ...
    if isPlot:
        ...
        if label:
            ...
            if Fs > 10**6:
                plt.text(text_position, -10, f'Fin/Fs = {bin_max/N*Fs/10**6:.1f} / {Fs/10**6:.1f} MHz')
            else:
                plt.text(text_position, -10, f'Fin/Fs = {bin_max/N*Fs/10**3:.1f} / {Fs/10**3:.1f} KHz')
