...
def signal_source(pr, mdl, v_in_peak):
    ...
    # mdl['is_verbose'] = 1
    pr['F_s'] = pr['F_clk'] / pr['task_frame'] * pr['TI']  # Sampling frequency for each input
    ...
    for iter_in in range(mdl['n_in_bus']):
        v_in_p[iter_in, :] = v_in_peak[iter_in] / 2 * np.cos(pr['F_in'][iter_in] * t_sim * 2 * np.pi)
        v_in_n[iter_in, :] = -v_in_peak[iter_in] / 2 * np.cos(pr['F_in'][iter_in] * t_sim * 2 * np.pi)
